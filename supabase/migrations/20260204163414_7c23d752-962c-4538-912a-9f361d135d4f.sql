...
-- Fix admin_reconcile_bepaid_legacy_subscriptions: orders table doesn't have profile_id column
-- Replace the function with a corrected version that uses NULL for profile_id from orders table
...
BEGIN
  ...
  -- would_insert = LEAST(missing_count, limit)
  ...
  -- FIX: orders table doesn't have profile_id, tariff_id, final_price - use NULL
  CREATE TEMP TABLE tmp_best_orders ON COMMIT DROP AS
  WITH all_orders AS (
    -- From orders_v2 with ranking (has profile_id)
    SELECT 
    ...
    -- From orders with ranking (NO profile_id, tariff_id, final_price columns!)
    SELECT 
      meta->>'bepaid_subscription_id' AS sbs_id,
      id AS order_id,
      'orders' AS order_src,
      user_id,
      NULL::uuid AS profile_id,  -- FIX: orders doesn't have profile_id
      product_id,
      NULL::uuid AS tariff_id,   -- FIX: orders doesn't have tariff_id
      amount AS final_price,     -- FIX: orders has 'amount' not 'final_price'
      currency,
      created_at,
      CASE 
...
REVOKE EXECUTE ON FUNCTION public.admin_reconcile_bepaid_legacy_subscriptions FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.admin_reconcile_bepaid_legacy_subscriptions FROM authenticated;
