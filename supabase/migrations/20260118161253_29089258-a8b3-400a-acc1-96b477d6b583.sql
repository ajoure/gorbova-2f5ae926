...
-- 1. Исправляем статус pending/processing транзакций - они НЕ должны быть successful
...
-- 2. Обновляем функцию статистики чтобы правильно считать:
-- - successful = только те где status = 'successful' или 'Успешный' или message = 'Successfully processed' И status = 'completed'
...
AS $function$
  SELECT json_build_object(
    -- Успешные: только реально завершённые платежи
    ...
    -- Неуспешные (исключая отмены и pending)
