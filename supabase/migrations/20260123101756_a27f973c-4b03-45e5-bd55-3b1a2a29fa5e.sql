-- PATCH 14.2: Пересоздать функцию find_wrongly_revoked_users

-- Сначала удаляем существующую функцию
DROP FUNCTION IF EXISTS find_wrongly_revoked_users();

-- Создаём заново с поддержкой всех источников доступа
CREATE OR REPLACE FUNCTION find_wrongly_revoked_users()
RETURNS TABLE (
  user_id uuid,
  profile_id uuid,
  full_name text,
